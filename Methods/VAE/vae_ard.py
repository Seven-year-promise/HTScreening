...
class VAE(nn.Module):
    ...
    def reparameterize(self, mu, log_var):
        ...
        """
        # z_loc = torch.zeros(mu.shape[0], latent_dim, dtype=mu.dtype, device=mu.device)
        # z_scale = torch.ones(mu.shape[0], latent_dim, dtype=mu.dtype, device=mu.device)
        # q_z = torch.normal(z_loc, z_scale)
        # q_kzz = self.rbf(q_z, q_z)
