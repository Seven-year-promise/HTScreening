...
def overlap_distribution(feature_mu={}, feature_sigma={}, labels={}, num_class = 130):
    mean_features = {}
    mean_features["train"] = []
    mean_features["eval"] = []
    feature_labels = {}
    feature_labels["train"] = []
    feature_labels["eval"] = []
    for c in range(num_class):
        # ind_vec = np.zeros_like(classes)
        # ind_vec[:, ic] = 1
        train_inds = labels["train"] == c
        eval_inds = labels["eval"] == c
        train_comp_mu = feature_mu["train"][train_inds]
        eval_comp_mu = feature_mu["eval"][eval_inds]
        train_comp_sigma = feature_sigma["train"][train_inds]
        eval_comp_sigma = feature_sigma["eval"][eval_inds]
        num_train_comp_data = train_comp_mu.shape[0]
        num_eval_comp_data = eval_comp_mu.shape[0]
        if (num_train_comp_data < 1) and (num_eval_comp_data < 1):
            continue

        mean_features["train"].append(np.average(train_comp_data, axis=0))
        mean_features["eval"].append(np.average(eval_comp_data, axis=0))
        feature_labels["train"].append(c)
        feature_labels["eval"].append(c)
