...
"""
"""
for each phase, the max and median values are selected
"""
...
def feature_extract(data):
    ...
    for i in range(len(PHASES)-1):
        ...
        l_q = np.max(phase_data) #np.quantile(phase_data, 0.25)
        h_q = np.median(phase_data) #np.quantile(phase_data, 0.75)
        features.append(l_q)
        features.append(h_q)
